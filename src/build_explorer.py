...
.rp-measure{max-width:84ch;margin-inline:auto}
</style>
</head>
<body>
<header>
...
function render1913(){
  ...
  const W=560,H=340,m={l:52,r:14,t:14,b:34};
  ...
  txt(s,m.l+8,m.t+14,"MLE xi 0.9801",{fill:"#5ad19a"});
  txt(s,m.l+8,m.t+30,"OLS xi 0.8553 (Ciccone recipe)",{fill:"#e8c468"});
  txt(s,m.l+8,m.t+46,"rank-1/2 OLS xi 0.8027",{fill:"#6fb3f2"});
  ...
  const W4=560,H4=320,m4={l:46,r:14,t:14,b:70};
...
function renderModern(){
  const W=560,H=380,m={l:64,r:64,t:20,b:30};
  ...
  txt(s,m.l-8,m.t-6,"1913",{anchor:"end",fill:"#9aa4b2"});
  txt(s,W-m.r+8,m.t-6,"modern",{anchor:"start",fill:"#9aa4b2"});
  pairs.forEach(p=>{
    const up=p.b>=p.a;
    ...
    txt(s,m.l-8,py(p.a)+4,p.name+" "+p.a,{anchor:"end",size:11});
    txt(s,W-m.r+8,py(p.b)+4,p.cc+" "+p.b,{size:11});});
  ...
  const W4=560,H4=260,m4={l:46,r:14,t:18,b:34};
  ...
  txt(s4,m4.l+6,m4.t+10,"admin 57.4-87.2 (ranks 15-131)",{fill:"#6fb3f2",size:11});
  txt(s4,m4.l+6,m4.t+25,"FUA 71.1-158.4 (ranks 15-89)",{fill:"#e8c468",size:11});
...
function renderMountains(){
  ...
  const draw=()=>{
    ...
    const W=900,H=400,m={l:58,r:16,t:16,b:38};
    ...
    if(showM1){
      ...
      drawLine(s,r0,pts.length,r=>a.h_min*Math.pow(r/r0,-a.xi),sc,"#e8c468","5 3");}
    drawPts(s,pts,sc,"#e6e6e6",k==="A0"?1.8:2.6);
    tickLabels(s,sc,W,H,m);
    const yl=sc.py(a.h_min);
    s.appendChild(el("line",{x1:m.l,y1:yl,x2:W-m.r,y2:yl,stroke:"#5ad19a","stroke-dasharray":"4 3"}));
    txt(s,W-m.r-4,yl-6,"selected h_min "+a.h_min+" m",{anchor:"end",fill:"#5ad19a"});
    txt(s,m.l+8,m.t+14,"xi "+a.xi.toFixed(4)+"  CI ["+a.ci[0].toFixed(4)+", "+a.ci[1].toFixed(4)+"]",
      {fill:"#e6e6e6",size:12.5});
    txt(s,m.l+8,m.t+30,"lane: "+a.lane,{fill:"#5ad19a",size:12});
    box.appendChild(s);
